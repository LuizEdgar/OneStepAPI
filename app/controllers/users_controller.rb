class UsersController < ApplicationController
  ...
  private
    ...
    def create_user_params
      case params[:kind]
      when "volunteer"
        params.permit(:name, :username, :email, :kind, :password, volunteer_attributes: [:about, :occupation, :birth_at, :gender, :period_availability, :volunteered, :cpf, :rg, :verified, {skill_ids: []}, {cause_ids: []} ], locations_attributes: [ :address_1, :address_2, :city, :state, :country, :postcode ], phones_attributes: [ :number, :kind])
      when "organization"
    ...
    def update_user_params
      case @user.kind
      when "volunteer"
        params.permit(:name, :username, :email, :password, volunteer_attributes: [:id, :about, :occupation, :birth_at, :gender, :period_availability, :volunteered, :cpf, :rg, :verified, {skill_ids: []}, {cause_ids: []} ], locations_attributes: [:id,  :address_1, :address_2, :city, :state, :country, :postcode, :_destroy], phones_attributes: [:id,  :number, :kind, :_destroy])
      when "organization"
